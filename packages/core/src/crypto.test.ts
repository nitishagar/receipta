...
describe('Ed25519 — serialize / deserialize round-trip', () => {
  it('round-trips a full key pair through bytes (keys/<key_id> storage)', () => {
    ...
    expect(Buffer.from(exportPublicKey(restored.publicKey)).equals(serialized.publicKey)).toBe(
      true,
    );
...
describe('key-pair JSON format (Phase 1 on-disk encoding)', () => {
  it('round-trips a full key pair through keyPairToJsonString / keyPairFromJsonString', () => {
    const kp = generateKeyPair();
    const text = keyPairToJsonString(kp);
    const restored = keyPairFromJsonString(text);
    expect(restored.keyId).toBe(kp.keyId);
    // Restored private key produces a signature that verifies under the original public key.
    const data = Buffer.from('round-trip', 'utf8');
    const sig = sign(data, restored.privateKey);
    expect(verify(data, sig, kp.publicKey)).toBe(true);
  });

  it('emits hex-encoded byte fields (not index-keyed objects from naive JSON.stringify)', () => {
    const kp = generateKeyPair();
    const text = keyPairToJsonString(kp);
    const obj = JSON.parse(text);
    expect(typeof obj.publicKey).toBe('string');
    expect(obj.publicKey).toMatch(/^[0-9a-f]+$/);
    expect(typeof obj.privateKey).toBe('string');
    expect(obj.privateKey).toMatch(/^[0-9a-f]+$/);
    expect(obj.keyId).toBe(kp.keyId);
  });

  it('rejects a key file whose keyId does not match the embedded public key (anti-substitution, S7)', () => {
    const kp = generateKeyPair();
    const text = JSON.stringify({
      keyId: 'deadbeef'.repeat(8),
      publicKey: toHex(exportPublicKey(kp.publicKey)),
      privateKey: toHex(serializeKeyPair(kp).privateKey),
    });
    expect(() => keyPairFromJsonString(text)).toThrow(/does not match fingerprint/);
  });

  it('rejects malformed JSON', () => {
    expect(() => keyPairFromJsonString('not json')).toThrow(/not valid JSON/);
  });

  it('rejects a non-object top level', () => {
    expect(() => keyPairFromJsonString('"a string"')).toThrow(/expected a JSON object/);
    expect(() => keyPairFromJsonString('42')).toThrow(/expected a JSON object/);
  });

  it('rejects missing or mistyped required fields', () => {
    expect(() => keyPairFromJsonString('{"foo":"bar"}')).toThrow(/missing required string fields/);
    expect(() => keyPairFromJsonString('{"keyId":"x","publicKey":1}')).toThrow(
      /missing required string fields/,
    );
    expect(() => keyPairFromJsonString('{"keyId":1,"publicKey":"ab"}')).toThrow(
      /missing required string fields/,
    );
  });

  it('rejects a non-string privateKey when present', () => {
    const kp = generateKeyPair();
    const text = JSON.stringify({
      keyId: kp.keyId,
      publicKey: toHex(exportPublicKey(kp.publicKey)),
    ...
    expect(() => keyPairFromJsonString(text)).toThrow(/privateKey must be a hex string/);
  });

  it('constructs a verify-only holder when privateKey is absent', () => {
    const kp = generateKeyPair();
    const text = JSON.stringify({
      keyId: kp.keyId,
      publicKey: toHex(exportPublicKey(kp.publicKey)),
