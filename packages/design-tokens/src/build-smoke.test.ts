...
function ensureBuild(): void {
  ...
  if (result.status !== 0) {
    throw new Error(`vitepress build failed (exit ${result.status})\n${result.stderr}`);
  }
}
...
describe('docs build contract (INV-2 / INV-9 / INV-1)', () => {
  ...
  it('references the IBM Plex font family in built CSS (the restyle took effect)', () => {
    ...
    const allCss = cssFiles.map((f) => readFileSync(resolve(assetsDir, f), 'utf8')).join('\n');
  ...
  it('every font url(...) resolves under the project base (INV-1, no broken absolute paths)', () => {
    ...
    for (const url of matches) {
      ...
      expect(isRelative || isBased || isData, `font url not base-prefixed: ${cleaned}`).toBe(true);
    }
  });
});
